...
export const useCartStore = create<CartState>((set) => ({
  cart: typeof window !== "undefined" ? JSON.parse(localStorage.getItem(CART_KEY) || "[]") : [],
          ...
          .then((response) => {
            ...
            set((currentState) => {
              const synced = currentState.cart.map((cartItem) =>
                cartItem.id === item.id ? { ...cartItem, cartItemId: apiItem.id } : cartItem,
              );
    ...
    set((state) => {
      const updated = state.cart.map((item) => {
        ...
        if (item.cartItemId && isLoggedIn()) {
          void http.patch(`/cart/items/${item.cartItemId}`, { quantity: newQuantity }).catch(() => {});
        }
