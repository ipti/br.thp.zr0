...
export default function ProductTransformationWorkshop({
  ...
  const header = (
   ...
   const textEditor = (options: any) => {
        return <ZInputText type="text" value={options.value} onChange={(e) => options.editorCallback(e.target.value)} />;
    };

    const textEditorNumber = (options: any) => {
        return <ZInputNumber pt={{input:{root:{style: {witdh: "32px"}}}}} value={parseInt(options.value)} onChange={(e) => options.editorCallback(e.value)} />;
    };
